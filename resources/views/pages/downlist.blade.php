...
@section('content')
	...
	<div class="row">
		...
		<div class="col-md-12">
			...
			<form method="GET" role="form" action="/downtime-list">
			...
			<ul class="page-breadcrumb breadcrumb">
				...
				<li>
					<select class="form-control input-sm" name="isplanned">
		 				<option value="3" {{request()->isplanned == 3?'selected':'' }}> - ALL - </option>
		 				<option value="1" {{request()->isplanned == 1?'selected':'' }}> PLANNED </option>
		 				<option value="0" {{request()->has('isplanned') && request()->isplanned == 0?'selected':'' }}> BREAKDOWN </option>
		 				<option value="2" {{request()->isplanned == 2?'selected':'' }}> GRID OUTAGE </option>	 					
		 			</select>
				</li>
				...
				<tbody>
					...
					@foreach( $downtime as $key => $down )
						<tr>
							...
							<td >
								@if( $down->is_scheduled  == 1 ) 
									Scheduled
								@elseif( $down->is_scheduled == 0 )
									Unscheduled
								@else
									Grid Outage
								@endif
							...
							<td >
								<a href="#" class="btn purple btn-sm" onclick="window.open('/downtime/{{$down->id}}','displayWindow','toolbar=no,scrollbars=yes,width=800,height=600'); return false; " title="Edit Downtime"><i class="fa fa-edit"></i></a>
		  	...
		  	TableAdvanced.init();
		   	...
		   	$('#sample_4').on('click','.deletedl', function(){       
                ...
                bootbox.confirm("Are you sure you want to delete this record?", function(result) {
                    ...
                    if(result){
                        // window.location = "index.php?delete=delete&id="+x;
                        $.ajaxSetup({
                            headers:
